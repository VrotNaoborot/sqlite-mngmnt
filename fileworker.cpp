...
bool FileWorker::file_exists(const std::string& filename) {
    // ф-ция для проверки существования файла
    std::string extension = ".db";
    if (filename.length() < extension.length()) {
        return false;
    }

    // проверяем, совпадает ли расширение файла с .db
    std::string file_extension = filename.substr(filename.length() - extension.length());
